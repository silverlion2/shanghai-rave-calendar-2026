...
const htmlFiles = ["index.html", "venues.html", "djs.html", "planner.html", "poster-wall.html", "ops.html"];
...
function assertGeneratedSeoPages(events) {
  ...
  if (!fs.existsSync(eventIndexFile)) {
    throw new Error("events/index.html must be generated by npm run seo");
  }
  const eventIndexHtml = fs.readFileSync(eventIndexFile, "utf8");
  assertSeoHeadMarkers(eventIndexFile, eventIndexHtml);
  const indexNodes = generatedEventPageGraph(eventIndexFile, eventIndexHtml);
  if (!indexNodes.some(node => node["@type"] === "CollectionPage")) {
    throw new Error("events/index.html missing CollectionPage JSON-LD graph node");
  }
  ...
  for (const loc of [`${siteUrl}/`, `${siteUrl}/events`, `${siteUrl}/planner`, `${siteUrl}/djs`, `${siteUrl}/venues`, `${siteUrl}/poster-wall`]) {
...
const scrapeRequirements = [
  ...
  { file: "index.html", text: 'href="events/index.html"', label: "calendar events index link" },
