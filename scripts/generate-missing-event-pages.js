// Generate missing event detail pages for events that exist in data/events.json
// but are missing events/<id>.html pages.
...
const MISSING_IDS = [
  ...
  'disco-ball-fridays-dome',
  'hush-three-year-anniversary-dj-david',
  'nova-sundown-char-bar-rooftop',
];

const TEMPLATE = fs.readFileSync('events/alter-pavillon.html', 'utf8');
...
function posterUrlFor(event) {
  const p = event.posterUrl;
  if (!p) return 'assets/posters/alter-pavillon-optimized.jpg';
  return p;
}
...
function buildPage(event) {
  ...
  const status = String(event.status || event.confidence || '').toLowerCase() === 'watch' || String(event.confidence || '').toLowerCase() === 'watch'
    ? 'watch / Watch'
    : 'upcoming / High';
  ...
  const lineup = Array.isArray(event.lineup) && event.lineup.length
    ...
    : `<li><b>Lineup TBA</b><span>${escapeHtml('DJ lineup was not detailed in the public source; check the event page or venue channel for the latest names.')}</span></li>`;
  ...
  const recReason = event.recommendationReason || `Recommended because ${escapeHtml(venue)} offers a social, rooftop/dancefloor-adjacent option that is useful as a companion to a pure-techno night, rather than as a dedicated club pick.`;
  const bestFor = event.bestFor || `Best for readers who want a more social/dancefloor-adjacent night at ${escapeHtml(venue)}.`;
  ...
  "@graph": [
    {
      ...
      "publisher": {
        ...
        "logo": {
          "@type": "ImageObject",
          "url": "https://raveindexsh.top/og-image.png"
        }
      }
    },
    {
      ...
      "itemListElement": [
        {
          ...
          "position": 1,
          "name": "Home",
          "item": "https://raveindexsh.top"
        },
        {
          "@type": "ListItem",
          "position": 2,
          "name": "Events",
          "item": "https://raveindexsh.top/poster-wall"
        },
        {
          "@type": "ListItem",
          "position": 3,
          "name": "${escapeHtml(title)}",
          "item": "https://raveindexsh.top/events/${escapeHtml(id)}"
        }
      ]
    },
    {
      ...
      "image": [
        "https://raveindexsh.top/${escapeHtml(poster)}"
      ],
      "location": {
        "@type": "Place",
        ...
        "address": {
          ...
          "addressCountry": "CN",
          "streetAddress": "${escapeHtml(address || venue)}"
        }
      },
      "sameAs": [
        "${escapeHtml(source)}"
      ...
      "organizer": {
        "@type": "Organization",
        "name": "${escapeHtml(organizer || venue)}"
      },
      "performer": [
        {
      ...
      <span>Basement Dispatch / source-first calendar</span>
        ...
        <article class="event-detail  ">
          <header class="event-hero">
            <div>
              <p class="kicker">${escapeHtml(status)}</p>
          ...
          <section class="facts" aria-label="Event facts">
            <div class="fact"><span>Date</span><b>${escapeHtml(date)}</b></div><div class="fact"><span>Time</span><b>${escapeHtml(time)}</b></div><div class="fact"><span>Venue</span><b>${escapeHtml(venue)}</b></div><div class="fact"><span>District</span><b>${escapeHtml(district)}</b></div><div class="fact"><span>Sound</span><b>${escapeHtml(sound)}</b></div><div class="fact"><span>Price</span><b>${escapeHtml(price)}</b></div><div class="fact"><span>Age / ID</span><b>${escapeHtml(age)}</b></div><div class="fact"><span>Source layer</span><b>${escapeHtml(event.sourceStatus || 'secondary')}</b></div><div class="fact"><span>Intensity</span><b>${escapeHtml(event.decisionProfile && event.decisionProfile.intensity ? event.decisionProfile.intensity : 'soft')}</b></div><div class="fact"><span>Credibility</span><b>${escapeHtml(event.decisionProfile && event.decisionProfile.credibility ? event.decisionProfile.credibility : 'unconfirmed')}</b></div>
          </section>
          ...
          <section class="copy-block recommendation-ledger" aria-label="How we recommend this event">
            ...
            <ul class="lineup-list">
              <li><b>Recommendation</b><span>${escapeHtml(recReason)}</span></li><li><b>Best for</b><span>${escapeHtml(bestFor)}</span></li><li><b>Verify before going</b><span>${escapeHtml(verifyBefore)}</span></li><li><b>Source confidence</b><span>${escapeHtml(sourceConfidence)}</span></li>
            </ul>
          </section>
          ...
          <section class="copy-block trust-ledger" aria-label="Trust ledger">
            <h2>Trust Ledger</h2>
            <ul class="lineup-list">
              <li><b>Last checked</b><span>${escapeHtml(lastChecked)}</span></li><li><b>Selection basis</b><span>Included for source visibility, room fit, and these editorial signals: ${escapeHtml((event.decisionTags || []).join(', '))}.</span></li><li><b>Commercial relationship</b><span>No paid placement recorded.</span></li><li><b>Correction route</b><span>Send source fixes through RED 980793145 or the corrections policy.</span></li>
...
for (const id of MISSING_IDS) {
  ...
  if (!event) {
    console.error(`Could not find event in data/events.json: ${id}`);
    continue;
  }
  const page = buildPage(event);
  const outPath = path.join('events', `${id}.html`);
  fs.writeFileSync(outPath, page, 'utf8');
  console.log(`Wrote ${outPath} (${page.length} chars)`);
}
console.log('Done.');
